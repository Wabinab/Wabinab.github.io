...
As you've noticed, if one had refused to install mathlive locally on the computer using `npm install`, and instead pull the script and css from CDN instead, everything would be solved. That's the easy way. Unfortunately, my client refused to connect the computer to the internet for security issues, so everything needs to run locally; and CDN is just not a viable option. Does this already spark your creativity? 

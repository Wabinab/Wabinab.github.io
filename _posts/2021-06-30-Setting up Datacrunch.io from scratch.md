...
However, these all requires root permission to access **which is annoying**, hence the best way is to use SSHFS to connect to it. There is a really good article written on how to do this [here](https://www.redhat.com/sysadmin/sshfs). Check it out! Now after this, you can access it with full permission (i.e. without root). Irregardless of how fast is the connection between host and your machine, you can now use the particular folder. 
